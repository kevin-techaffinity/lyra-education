...
import { createSubscription, getAlertContent, getSlots } from '../../services/CourseSubscription';
...
const staticContent = {
  ...
  appendMessage: " Are you sure you want to take this course as your next? You can press on Got it",
  buttonLabel: "Proceed",
}
...
export default function ModuleHeader({ slug, item }) {
  ...
  const populateAlertScreens = async () => {
    const data = await getAlertContent(clientDomain);
    setContent(data[0]);
  }
        ...
        <Modal.Body>
          <div>
            ...
            <div className="modal-main-content">
              ...
              <h4>{render?.title}</h4>
              <p>
                You now have <strong className="font-weight-bold">{remainingSlot}</strong> {render?.appendMessage}
              </p>
            </div>
